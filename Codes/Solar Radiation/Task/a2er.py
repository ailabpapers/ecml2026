...
class multiTaskNN(torch.nn.Module):
  def __init__(self):
      ...
      self.task1 = torch.nn.Linear(44, 1)
      self.task2 = torch.nn.Linear(44, 1)
      self.task3 = torch.nn.Linear(44, 1)
      self.task4 = torch.nn.Linear(44, 1)
      self.task5 = torch.nn.Linear(44, 1)
      self.task6 = torch.nn.Linear(44, 1)
      self.task7 = torch.nn.Linear(44, 1)
      self.task8 = torch.nn.Linear(44, 1)
...
df_curr = df.loc[df['City'] == 'boston']

task_IL_targets = ['Clearsky DNI', 'Clearsky DHI', 'Clearsky GHI', 'DNI','DHI', 'GHI', 'Global Horizontal UV Irradiance (280-400nm)', 'Global Horizontal UV Irradiance (295-385nm)']
...
def train(dataloader, val_data, model, loss_fn, optimizer, alpha_raw, beta_logit, N_RS, rho, eps, lambda_priority, delta_Q, task_id):
  ...
  for i, (X, y) in enumerate(dataloader):
    ...
    if len(reservoir_buffer) > 0:
      ...
      feat_rs = feat_rs.unsqueeze(0)  # [1, 4]
      saved_logit_rs = saved_logit_rs.view(1) # [1]
      model.eval()
      with torch.no_grad():
        buffer_pred_rs = model(feat_rs, task_id)
